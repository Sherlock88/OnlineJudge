...
int main()
{
	unsigned long N, T, bandit[100001], i, cycleCount, cycleLength[100000], cycleStart, nextJump, cyclePtr;
	...
	{
		scanintUL(N);
		for(i = 1; i <= N; i++) scanintUL(bandit[i]);
		...
		{
			cyclePtr = cycleStart;
			cycleLength[cycleCount] = 1;
			while(1)
			{
				nextJump = bandit[cyclePtr];
				bandit[cyclePtr] = 0;
				if(nextJump == cycleStart) break;
				cycleLength[cycleCount]++;
				cyclePtr = nextJump;
			}
			cycleCount++;
			for(; (cycleStart <= N) && (!bandit[cycleStart]); cycleStart++);
		}
		
		printf("%lu\n", lcmModulo(cycleLength, cycleCount));
	}
	return 0;
}